...
def test_move_up():
    ...
    assert puzzle.move_up() == [0, 2, 3, 4, 1, 6, 7, 5]

    puzzle = Puzzle(2, [1, 2, 3, 4, 6, 0, 7, 5])
    assert puzzle.move_up() == [1, 0, 3, 4, 6, 2, 7, 5]

    puzzle = Puzzle(3, [1, 2, 3, 4, 6, 0, 7, 5 , 8])
    assert puzzle.move_up() == [1, 2, 0, 4, 6, 3, 7, 5 , 8]

    puzzle = Puzzle(3, [1, 2, 3, 4, 6, 8, 7, 0, 5])
    assert puzzle.move_up() == [1, 2, 3, 4, 0, 8, 7, 6, 5]
...
def test_move_down():
    ...
    assert puzzle.move_down() == [1, 2, 3, 5, 4, 6, 7, 0]

    puzzle = Puzzle(2, [1, 0, 3, 4, 6, 2, 7, 5])
    assert puzzle.move_down() == [1, 2, 3, 4, 6, 0, 7, 5]

    puzzle = Puzzle(3, [1, 2, 0, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_down() == [1, 2, 3, 4, 6, 0, 7, 5, 8]

    puzzle = Puzzle(3, [1, 2, 3, 4, 0, 8, 7, 6, 5])
    assert puzzle.move_down() == [1, 2, 3, 4, 6, 8, 7, 0, 5]
...
def test_move_right():
    puzzle = Puzzle(2, [1, 2, 3, 4, 0, 6, 7, 5])
    assert puzzle.move_right() == [1, 2, 3, 4, 6, 0, 7, 5]

    puzzle = Puzzle(2, [1, 0, 3, 4, 6, 2, 7, 5])
    assert puzzle.move_right() == [1, 3, 0, 4, 6, 2, 7, 5]

    puzzle = Puzzle(2, [0, 1, 3, 4, 6, 2, 7, 5])
    assert puzzle.move_right() == [1, 0, 3, 4, 6, 2, 7, 5]

    puzzle = Puzzle(2, [5, 1, 3, 4, 6, 2, 0, 7])
    assert puzzle.move_right() == [5, 1, 3, 4, 6, 2, 7, 0]

    puzzle = Puzzle(3, [1, 0, 2, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_right() == [1, 2, 0, 4, 6, 3, 7, 5, 8]

    puzzle = Puzzle(3, [1, 2, 3, 4, 0, 8, 7, 6, 5])
    assert puzzle.move_right() == [1, 2, 3, 4, 8, 0, 7, 6, 5]
...
def test_move_left():
    puzzle = Puzzle(2, [1, 2, 3, 4, 6, 0, 7, 5])
    assert puzzle.move_left() == [1, 2, 3, 4, 0, 6, 7, 5]

    puzzle = Puzzle(2, [1, 3, 0, 4, 6, 2, 7, 5])
    assert puzzle.move_left() == [1, 0, 3, 4, 6, 2, 7, 5]

    puzzle = Puzzle(2, [1, 0, 3, 4, 6, 2, 7, 5])
    assert puzzle.move_left() == [0, 1, 3, 4, 6, 2, 7, 5]

    puzzle = Puzzle(2, [5, 1, 3, 4, 6, 2, 7, 0])
    assert puzzle.move_left() == [5, 1, 3, 4, 6, 2, 0, 7]

    puzzle = Puzzle(3, [1, 2, 0, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_left() == [1, 0, 2, 4, 6, 3, 7, 5, 8]

    puzzle = Puzzle(3, [1, 2, 3, 4, 8, 0, 7, 6, 5])
    assert puzzle.move_left() == [1, 2, 3, 4, 0, 8, 7, 6, 5]
...
def test_move_wrapping():
    ...
    assert puzzle.move_wrapping() == [1, 2, 3, 4, 0, 5, 7, 6]

    puzzle = Puzzle(2, [1, 2, 3, 4, 0, 5, 7, 6])
    assert puzzle.move_wrapping() == [1, 2, 3, 4, 6, 5, 7, 0]

    puzzle = Puzzle(2, [1, 2, 3, 0, 6, 5, 7, 4])
    assert puzzle.move_wrapping() == [0, 2, 3, 1, 6, 5, 7, 4]

    puzzle = Puzzle(2, [0, 2, 3, 1, 6, 5, 7, 4])
    assert puzzle.move_wrapping() == [1, 2, 3, 0, 6, 5, 7, 4]

    puzzle = Puzzle(3, [1, 2, 0, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_wrapping() == [0, 2, 1, 4, 6, 3, 7, 5, 8]

    puzzle = Puzzle(3, [1, 2, 7, 4, 6, 3, 0, 5, 8])
    assert puzzle.move_wrapping() == [1, 2, 7, 4, 6, 3, 8, 5, 0]

    puzzle = Puzzle(3, [1, 2, 3, 4, 8, 5, 7, 6, 0])
    assert puzzle.move_wrapping() == [1, 2, 3, 4, 8, 5, 0, 6, 7]
...
def test_move_diagonal_short():
    ...
    puzzle = Puzzle(2, [1, 2, 3, 4, 6, 5, 7, 0])
    assert puzzle.move_diagonal_short() == [1, 2, 0, 4, 6, 5, 7, 3]

    puzzle = Puzzle(2, [1, 2, 3, 4, 0, 5, 7, 6])
    assert puzzle.move_diagonal_short() == [1, 0, 3, 4, 2, 5, 7, 6]

    puzzle = Puzzle(2, [1, 2, 3, 0, 6, 5, 7, 4])
    assert puzzle.move_diagonal_short() == [1, 2, 3, 7, 6, 5, 0, 4]

    puzzle = Puzzle(2, [0, 2, 3, 1, 6, 5, 7, 4])
    assert puzzle.move_diagonal_short() == [5, 2, 3, 1, 6, 0, 7, 4]

    puzzle = Puzzle(3, [1, 2, 0, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_diagonal_short() == [1, 2, 6, 4, 0, 3, 7, 5, 8]

    puzzle = Puzzle(3, [1, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 0])
    assert puzzle.move_diagonal_short() == [1, 2, 7, 4, 6, 3, 0, 5, 8, 9, 10, 11]

    puzzle = Puzzle(4, [1, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 0])
    assert puzzle.move_diagonal_short() == [1, 2, 7, 4, 6, 3, 11, 0, 8, 9, 10, 5]

    puzzle = Puzzle(4, [1, 2, 7, 4, 6, 3, 11, 5, 8, 0, 10, 9])
    assert puzzle.move_diagonal_short() == [1, 2, 7, 4, 6, 3, 11, 0, 8, 5, 10, 9]

    puzzle = Puzzle(4, [1, 2, 0, 4, 6, 3, 11, 5, 8, 7, 10, 9])
    assert puzzle.move_diagonal_short() == [1, 2, 6, 4, 0, 3, 11, 5, 8, 7, 10, 9]
...
def test_move_diagonal_long():
    ...
    puzzle = Puzzle(2, [1, 2, 3, 4, 6, 5, 7, 0])
    assert puzzle.move_diagonal_long() == [0, 2, 3, 4, 6, 5, 7, 1]

    puzzle = Puzzle(2, [1, 2, 3, 4, 0, 5, 7, 6])
    assert puzzle.move_diagonal_long() == [1, 2, 3, 0, 4, 5, 7, 6]

    puzzle = Puzzle(2, [1, 2, 3, 0, 6, 5, 7, 4])
    assert puzzle.move_diagonal_long() == [1, 2, 3, 6, 0, 5, 7, 4]

    puzzle = Puzzle(2, [0, 2, 3, 1, 6, 5, 7, 4])
    assert puzzle.move_diagonal_long() == [4, 2, 3, 1, 6, 5, 7, 0]

    puzzle = Puzzle(3, [1, 2, 0, 4, 6, 3, 7, 5, 8])
    assert puzzle.move_diagonal_long() == [1, 2, 7, 4, 6, 3, 0, 5, 8]

    puzzle = Puzzle(3, [1, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 0])
    assert puzzle.move_diagonal_long() == [0, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 1]

    puzzle = Puzzle(4, [1, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 0])
    assert puzzle.move_diagonal_long() == [0, 2, 7, 4, 6, 3, 11, 5, 8, 9, 10, 1]

    puzzle = Puzzle(4, [1, 2, 0, 4, 6, 3, 11, 5, 7, 8, 10, 9])
    assert puzzle.move_diagonal_long() == [1, 2, 8, 4, 6, 3, 11, 5, 7, 0, 10, 9]

    puzzle = Puzzle(4, [1, 2, 7, 4, 6, 3, 11, 5, 8, 0, 10, 9])
    assert puzzle.move_diagonal_long() == [1, 2, 0, 4, 6, 3, 11, 5, 8, 7, 10, 9]
